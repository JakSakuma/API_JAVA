...
public class API {
	...
	public Response getVersion(@Context HttpServletRequest req) {
		...
		// Para testar em sua maquina, altere o arquivo swagger para ter o seu IP em
		// WebContent/WEB-INF/classes/Docs
		...
		try {
			...
			input = new Scanner(file, ENCODING_FORMAT);			
		...
		return Response.status(200).header(ACCESS_CONTROL, "*")
				.header("Access-Control-Allow-Methods", "GET, POST, DELETE, PUT, PATCH, OPTIONS")
				.header("Access-Control-Allow-Headers", "Content-Type, api_key, Authorization, *")
				.entity(json.toString()).encoding(ENCODING_FORMAT).type(MediaType.APPLICATION_JSON).build();
	}
}